...
# ============ Utils and augs ============#
from utils.BDataset import BDataset, BDatasetResize, resolution_dict, BDatasetResizeSeed, BDatasetResizeSeedErode
...
def main():
    ...
    if (args.predict or args.predict_train):
        ...
        for test_resl in test_resl_list:
            ...
            predict_dataset = BDatasetResizeSeedErode(df=test_df,
    ...
    # evaluate loop
        ...
        for resl_key, source_resl, target_resl in resolution_list:
            ...
            val_dataset = BDatasetResizeSeedErode(df=train_df,
                                                  transforms=val_augs,
        ...
        val_loss_avg = np.inner(np.asarray(val_losses), np.asarray(val_dset_lengths)) / np.asarray(
            val_dset_lengths).sum()
        val_f1_avg = np.inner(np.asarray(val_f1_scores), np.asarray(val_dset_lengths)) / np.asarray(
            val_dset_lengths).sum()
        val_map_avg_wt = np.inner(np.asarray(val_map_scores_wt), np.asarray(val_dset_lengths)) / np.asarray(
            val_dset_lengths).sum()
        val_map_avg_wt_seed = np.inner(np.asarray(val_map_scores_wt_seed), np.asarray(val_dset_lengths)) / np.asarray(
            val_dset_lengths).sum()
        ...
        for epoch in range(args.start_epoch, args.epochs):
            ...
            train_loss_avg = np.inner(np.asarray(train_losses), np.asarray(train_dset_lengths)) / np.asarray(
                train_dset_lengths).sum()
            val_loss_avg = np.inner(np.asarray(val_losses), np.asarray(val_dset_lengths)) / np.asarray(
                val_dset_lengths).sum()

            train_f1_avg = np.inner(np.asarray(train_f1_scores), np.asarray(train_dset_lengths)) / np.asarray(
                train_dset_lengths).sum()
            val_f1_avg = np.inner(np.asarray(val_f1_scores), np.asarray(val_dset_lengths)) / np.asarray(
                val_dset_lengths).sum()
            ...
            val_map_avg_wt = np.inner(np.asarray(val_map_scores_wt), np.asarray(val_dset_lengths)) / np.asarray(
                val_dset_lengths).sum()
            val_map_avg_wt_seed = np.inner(np.asarray(val_map_scores_wt_seed),
                                           np.asarray(val_dset_lengths)) / np.asarray(val_dset_lengths).sum()
...
def validate(val_loader,
    ...
    for i, (input, target, or_resl, target_resl, img_sample) in enumerate(val_loader):
        ...
        for j, pred_output in enumerate(output):
            ...
            mask_glob = '../data/military_data/masks/{}.tif'.format(img_sample[j])
            ...
            y_pred_wt = cv2.applyColorMap((y_pred_wt / (y_pred_wt.max() + 0.0001) * 255).astype('uint8'),
                                          cv2.COLORMAP_JET)
            y_pred_wt_seed = cv2.applyColorMap((y_pred_wt_seed / (y_pred_wt_seed.max() + 0.0001) * 255).astype('uint8'),
                                               cv2.COLORMAP_JET)
            ...
            if i == 0:
                if args.channels == 5:
                    ...
                    info = {
                        ...
                        'gt_vectors': to_np(target[:2, 6, :, :] + target[:2, 7, :, :]),
                    # simple hack - just sum the vectors
                        'pred_mask': to_np(m(output.data[:2, 0, :, :])),
                        'pred_mask1': to_np(m(output.data[:2, 1, :, :])),
                        'pred_mask2': to_np(m(output.data[:2, 2, :, :])),
                        'pred_mask3': to_np(m(output.data[:2, 3, :, :])),
